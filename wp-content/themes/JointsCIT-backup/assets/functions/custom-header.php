 ...
 */
function mro_cit_custom_header_setup() {
	 ...
	 * }
	 */
	add_theme_support( 'custom-header', apply_filters( 'mro_cit_custom_header_args', array(
		'default-image'      => get_parent_theme_file_uri( '/assets/images/hero-cit.jpg' ),
		'width'              => 2000,
		'height'             => 1062,
		'flex-height'        => true,
		'video'              => true,
		'wp-head-callback'   => 'mro_cit_header_style',
	) ) );
...
add_action( 'after_setup_theme', 'mro_cit_custom_header_setup' );

if ( ! function_exists( 'mro_cit_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see twentyseventeen_custom_header_setup().
 */
function mro_cit_header_style() {
 ...
 */
function mro_cit_video_controls( $settings ) {
...
add_filter( 'header_video_settings', 'mro_cit_video_controls' );
...
add_action('customize_register', 'mro_cit_hero_customizer');
function mro_cit_hero_customizer($wp_customize) {
	//adding section in wordpress customizer   
	// $wp_customize->add_section('hero_settings_section', array(
	// 	'title'          => 'Hero Text Section'
	// ));
	//adding setting for footer text area
	$wp_customize->add_setting('cit_hero_text', array(
		'default'        => 'Default Text For Hero Section',
	));
	$wp_customize->add_control('cit_hero_text', array(
		'label'   => 'Hero Text Here',
